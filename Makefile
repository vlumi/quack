...
help:  ## List the available commands
	@echo "Quack — available make targets:"
	@awk 'BEGIN {FS = ":.*## "} \
		/^##@ / {printf "\n\033[1m%s\033[0m\n", substr($$0, 5); next} \
...
Quack.xcodeproj: $(PROJECT_INPUTS)
	@Scripts/generate.sh

.PHONY: generate
generate: Quack.xcodeproj  ## Regenerate Quack.xcodeproj from project.yml (if stale)

.PHONY: run-mac
run-mac: Quack.xcodeproj  ## Build + launch the macOS app
	@Scripts/run.sh

.PHONY: run-iphone
run-iphone: Quack.xcodeproj  ## Build + launch on an iPhone simulator (DEVICE="SE" / "17 Pro" to pick)
	@Scripts/run-ios.sh iphone "$(DEVICE)"

.PHONY: run-ipad
run-ipad: Quack.xcodeproj  ## Build + launch on an iPad simulator (DEVICE="Air" / "13-inch" to pick)
	@Scripts/run-ios.sh ipad "$(DEVICE)"

.PHONY: build-mac
build-mac: Quack.xcodeproj  ## Build the macOS app
	@Scripts/build.sh macos

.PHONY: build-ios
build-ios: Quack.xcodeproj  ## Build the iOS app (simulator)
...
clean:  ## Remove the generated project + local build output
	@rm -rf Quack.xcodeproj .build-xcode
	@echo "removed Quack.xcodeproj and .build-xcode"
