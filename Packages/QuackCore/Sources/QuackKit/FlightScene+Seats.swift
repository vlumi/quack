...
extension FlightScene {
    ...
    func show(_ event: HazardEvent, at now: TimeInterval) {
        ...
        case .hit(let x, let y):
            ...
            flash = (
                practice.engineShotOut(at: localSeat)
        ...
        case .enemyHit(let down):
            ...
            if let i = practice.pilots.firstIndex(where: { $0.brain == .rival }),
    ...
    func resetSeats() {
        ...
        seatBulletNodes = [:]
        seatMarkers = [:]
        for (i, p) in practice.pilots.enumerated() where i != localSeat {
