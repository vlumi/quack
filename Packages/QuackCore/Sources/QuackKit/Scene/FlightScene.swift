...
public final class FlightScene: SKScene {
    ...
    var me: Pilot { run.pilots[min(localSeat, run.pilots.count - 1)] }
    ...
    public internal(set) var mode = Run.Mode.courier
    ...
    func applyTuning() {
        if session == nil { run.apply(tuning) }
        ...
        fieldNodes = run.model.strip.airfields.map { field in
            // Drawn with the field starting at 0; `render` places it.
            let local = Airfield(start: 0, length: field.length, name: field.name)
            let node = SKNode()
            let cones = SceneArt.approachCones(run.model, field: local, scale: scale)
            cones.name = "cones"
            node.addChild(cones)
            node.addChild(SceneArt.airfieldNode(local, scale: scale, palette: look.palette))
            SceneArt.setWindsock(
                in: node, step: run.windStep, direction: run.windDirection)
            fieldLayer.addChild(node)
            return node
        }
        minimap.reset(
            strip: run.model.strip,
            balloonColours: run.balloons.indices.map { look.palette.balloon($0) })
        resetTallies()
        resetHazards()
        resetSeats()
    }
    ...
    private func applyLook() {
        let hour = tuning.timeOfDay(seeded: run.hour)
        guard look.needsBuild(run, hour: hour) else { return }
        look.build(run, hour: hour, scale: scale, box: size)
        backgroundColor = look.palette.sky[2].color()
        hudInk = look.palette.hudInk
        applyInk()
        balloonNodes.forEach { $0.removeFromParent() }
        balloonNodes = run.balloons.enumerated().map { i, b in
    ...
    public func start(_ mode: Run.Mode) {
        guard mode != self.mode || run.startedAt != nil else { return }
        self.mode = mode
        nextSeed += 1
        startRun()
    }
    ...
    private func follow(_ plane: PlaneState) {
        ...
        // the ground.
        let below =
            [-30.0, 0, 30].map { run.model.strip.groundHeight(at: plane.x + $0) }.min() ?? 0
