...
extension FlightScene {
    ...
    func publishHUD() {
        ...
        if hud.board != run.offers { hud.board = run.offers }
        if hud.chosen != run.chosenOffer { hud.chosen = run.chosenOffer }
        if hud.carrying != run.contract { hud.carrying = run.contract }
        let names = run.model.strip.airfields.map(\.name)
    ...
    func resetTallies() {
        balloonTally.reset(colours: run.balloons.indices.map { look.palette.balloon($0) })
        roundTally.reset(
            colours: Array(
    ...
    func name(_ field: Int) -> String { run.model.strip.airfields[field].name }
    ...
    func updateHUD(at now: TimeInterval) {
        ...
        } else if run.mode == .courier {
            countLabel.text = francs(run.money)
            if let contract = run.contract, let pay = run.payNow {
                clockLabel.text = job(contract) + ": " + francs(pay)
            } else if let pick = run.chosen {
    ...
    private func updateAmmo() {
        ...
        } else if run.isRearming(at: localSeat) {
    ...
    private func status(at now: TimeInterval) -> String {
        ...
        case .parked
        where run.isRefuelling(at: localSeat) || run.isRearming(at: localSeat):
            return run.isRefuelling(at: localSeat)
