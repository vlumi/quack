...
final class StripLook {
    ...
    func needsBuild(_ run: Run, hour: TimeOfDay) -> Bool {
        built?.seed != run.seed || built?.hour != hour
            || builtScenery != run.model.strip.scenery
            || hangarNodes.count != run.model.strip.airfields.count
    }

    func build(_ run: Run, hour: TimeOfDay, scale: CGFloat, box: CGSize) {
        ...
        backdrop.build(
            Backdrop.generate(seed: run.seed, stripLength: strip.length), palette: palette,
            scale: scale)
    ...
    func update(
        _ run: Run, planePoints: CGPoint, cameraY: CGFloat, scale: CGFloat, box: CGSize
    ) {
        let strip = run.model.strip
        let planeX = Double(planePoints.x / scale)
        sky.update(
            planePoints: planePoints.x, driftPoints: CGFloat(run.airDrift) * scale,
        ...
        for (node, cloud) in zip(cloudNodes, run.clouds) {
