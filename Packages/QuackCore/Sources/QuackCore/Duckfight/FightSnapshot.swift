...
public struct FightSnapshot: Equatable, Sendable {
    ...
    public init(of run: Run, tick: Int) {
        self.tick = tick
        elapsed = run.elapsed
        finished = run.isFinished
        seats = run.pilots.map { p in
            let doing: Doing
            if p.down {
                doing = .down
            } else if p.falling {
    ...
    public func apply(to run: inout Run) {
        for (i, s) in seats.enumerated() where run.pilots.indices.contains(i) {
            var p = run.pilots[i]
            p.plane = s.plane
            p.health = s.health
            p.kills = s.kills
        ...
        if finished, run.finishedAt == nil { run.finishedAt = run.time }
    }
