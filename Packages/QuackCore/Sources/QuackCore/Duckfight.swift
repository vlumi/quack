...
extension Practice {
    ...
    mutating func seatTheDuckfight() {
        ...
        for k in 0..<o.rivals {
            ...
            var r = Practice.rival(strip: strip, tuning: enemyTuning, health: o.health)
            r.plane.x = centre
            r.plane.y = strip.groundHeight(at: centre) + 70
            r.patrol = (centre - enemyTuning.patrolHalf)...(centre + enemyTuning.patrolHalf)
    ...
    mutating func fall(at i: Int, dt: Double) {
        ...
        if e.plane.y <= strip.surfaceHeight(at: e.plane.x) + 1 {
            ...
            hazardEvent = e.brain == .rival ? .enemyDown : .downed(seat: i, by: nil)
        }
        pilots[i] = e
    }
