...
final class HazardTests: XCTestCase {
    ...
    func testACourierRunHasThreeGunsClearOfTheFieldsAndTheBalloonRunNone() {
        let p = Practice(seed: 1, mode: .courier)
        ...
        XCTAssertEqual(p.guns, Practice(seed: 1, mode: .courier).guns, "seeded")
        XCTAssertNotEqual(p.guns, Practice(seed: 2, mode: .courier).guns)
        XCTAssertTrue(Practice(seed: 1, mode: .balloons).guns.isEmpty)
    }
    ...
    func testOutOfRangeOrOnTheGroundNothingFires() {
        ...
        var parked = Practice(seed: 1, mode: .courier)
