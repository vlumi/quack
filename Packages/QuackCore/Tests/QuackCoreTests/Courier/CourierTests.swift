...
final class CourierTests: XCTestCase {
    ...
    private func day(seed: UInt64 = 1) -> Run {
    ...
    private func park(_ p: inout Run, at index: Int) {
    ...
    func testFieldsHaveSeededDistinctVillageNames() {
        let names = Run(seed: 3).model.strip.airfields.map(\.name)
        ...
        XCTAssertEqual(names, Run(seed: 3).model.strip.airfields.map(\.name))
        XCTAssertNotEqual(names, Run(seed: 4).model.strip.airfields.map(\.name))
        XCTAssertEqual(
    ...
    private func takeOff(_ p: inout Run) -> Contract {
        let pick = p.chosen!
    ...
    func testTheBalloonRunHasNoBoardAndTheDefaultRunIsBalloons() {
        ...
        XCTAssertFalse(Run(seed: 1, mode: .courier).needsToLand)
    }
}
