...
final class CareerTests: XCTestCase {
    ...
    func testARunStartsFromTheCareerWithItsUpgradesAndThePanelKeepsThem() {
        ...
        c.buy(.engine)
        c.buy(.seat)
        let stock = Practice(seed: 1, mode: .courier)
        var p = Practice(seed: 1, mode: .courier, career: c)
        ...
        XCTAssertEqual(
            Practice(seed: 1, mode: .balloons, career: c).money, 0, "the balloon run has no till")
    }

    func testRoundsCostMoneyAtTheFieldForTheCourierOnly() {
        var p = Practice(seed: 1, mode: .courier, career: Career(money: 10))
        p.ammo = 0
        ...
        var broke = Practice(seed: 1, mode: .courier)
        broke.ammo = 0
