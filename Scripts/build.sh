...
platform="${1:-macos}"
case "$platform" in
    macos) scheme="Quack-macOS"; destination="platform=macOS" ;;
    ios) scheme="Quack-iOS"; destination="generic/platform=iOS Simulator" ;;
...
if command -v xcbeautify >/dev/null; then
    set -o pipefail
    xcodebuild -project Quack.xcodeproj -scheme "$scheme" \
