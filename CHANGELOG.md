...
- Wind: each run has a steady wind of up to 16 m/s one way or the other, so flying round the strip is quicker one way. Your plane drifts with it while you fly; the landing assist and the ground do not, so landings still stop on the field. There is no gauge: the clouds drift with it, the windsocks point downwind, and the balloons drift slowly, rising over hills. Strength and balloon drift are on the tuning panel.
