...
How Quack Express versions, builds, and ships. Mechanical steps only — the
...
Both targets (`Quack-iOS`, `Quack-macOS`) carry their own copy of these. They're
**one app across two platforms** (§ Platforms — a Universal Purchase sharing the
bundle ID `fi.misaki.quack`). The release lane keeps both numbers **in lock-step**:
...
both platforms share the bundle ID `fi.misaki.quack`, the same Apple ID, SKU, and
...
team are already set on `Quack-macOS`.
